...
template <typename T>
void ClassInfo<T>::Deserialize(void* pClass, JsonReader* const reader, SerializationMap& context)
{
	for (auto const& pair : Properties)
	{
	...
	if (pBaseClass != nullptr)
	{
		pBaseClass->Deserialize(pClass, reader, context);
	}
}
...
void Reflection::DeserializeClass(T* pClass, JsonReader* const reader, SerializationMap& context) const
{
	...
	m_ClassRegistry.at(typeName)->Deserialize(pClass, reader, context);
}
